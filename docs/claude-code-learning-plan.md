...
- [x] フェーズ1: 基盤の更新
- [x] フェーズ2: Hooksの高度な活用（完了 - `docs/phase2-progress.md`参照）
...
**重要:** フェーズ1で扱うCLAUDE.mdとrulesシステムには、2026年2月時点で複数の既知の不具合があります。

**📄 詳細:** [既知の不具合と注意事項](./issues/known-issues.md)

主な不具合:
- **パスベースのルールがWrite操作で適用されない**（重大） - [#23478](https://github.com/anthropics/claude-code/issues/23478)
- **CLAUDE.mdのルールが無視される** - [#19635](https://github.com/anthropics/claude-code/issues/19635)
- **`.claudeignore`が機能しない**（セキュリティ問題）
- **設定の構文と挙動の問題**（絶対パスは`//`で始める必要がある）
- **品質劣化**（2026年1月下旬以降） - [#21431](https://github.com/anthropics/claude-code/issues/21431)

詳細な説明と回避策については、上記のドキュメントを参照してください。
...
1. **ターミナル通知フック（Stop）** - DevContainer環境向け
```bash
#!/bin/bash
# ~/.claude/hooks/notify-stop.sh
# DevContainer（Linux）環境では osascript が使えないため、
# ターミナルベル音と視覚的メッセージで通知
...
> **Note:** macOSのネイティブ環境では`osascript`を使った通知も可能ですが、DevContainer内ではLinuxコンテナのため利用できません。
...
**検証:**
- Stop通知: Claudeにタスクを依頼し、完了時に通知が表示されることを確認
...
いきなりフェーズを開始せずに、現在までに実施したこと、これからやることを生徒に伝えて、学習を開始するか訪ねてください。
生徒から「お願いします」と言われたら開始してください。
